...
import { InsertUser, users, portfolioProjects, contactMessages, InsertContactMessage } from "../drizzle/schema";
...
export async function getPortfolioProjects(category?: string): Promise<any[]> {
  const db = await getDb();
  if (!db) {
  ...
  try {
    let query: any = db.select().from(portfolioProjects);
    if (category) {
      query = query.where(eq(portfolioProjects.category, category as any));
    }
    return await query.orderBy(portfolioProjects.orderIndex);
  } catch (error) {
